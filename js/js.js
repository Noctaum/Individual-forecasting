	    ...
	    for(var j=0; j<((+cols)+2); j++){
	    	if(i === 0) {
	    		if(j === 0){
	    			...
	    			var colHead = $('<td><h5>Клаcс K</h5></td>');
	    			rowHead.append(colHead);
...
//Переписывание значений из таблицы в объект
	...
	$('#thirdZero').append('<p><b>Выберите метод определения хi0</b></p>');
  	$('#thirdZero').append('<p><input class="rad" name="metod" type="radio" checked> Среднее значение МО</p>');
   	$('#thirdZero').append('<p><input class="rad" name="metod" type="radio"> Средняя точка зоны перекрытия</p>');
  	$('#thirdZero').append('<p><input class="rad" name="metod" type="radio"> Информационный подход</p>');
  	$('#thirdZero').append('<p><input class="rad" name="metod" type="radio"> Методом прогнозирования</p>');
	...
	function case3(){
		...
		for (var i=0; i<cols; i++){
			...
			if (sumK1[i] >= sumK2[i]) {               //используем правило if(xi>+x0)z=1;
				...
				pieceFromPice[i] = (+piece[i])/16;
				minValueSumK[i] = (+sumK2[i]);

				for (var seven = 0; seven<16; seven++){
		...
		for (i=0; i<cols; i++){
			...
			for (seven = 0; seven<16; seven++){
				...
				megaK1Xi0[i][seven] = (0-1)*((+megaP1[i][seven])*(((+megaPK1Z1[i][seven])*Math.log((+megaPK1Z1[i][seven])))+((+megaPK2Z1[i][seven])*Math.log((+megaPK2Z1[i][seven])))))-((+megaP0[i][seven])*(((+megaPK1Z0[i][seven])*Math.log((+megaPK1Z0[i][seven])))+((+megaPK2Z0[i][seven])*Math.log((+megaPK2Z0[i][seven])))));
		...
		maxF = ((+minValueSumK[trator])+((+pieceFromPice[trator])*(16)));
		...
		for(var j=0; j<cols*2+3; j++){
		    if (i === 0){
		    	if(j === cols*2+2){
		    		...
		    		var colHead = $('<td><h5>Класс К</h5></td>');
		    		rowHead.append(colHead);
		    	}
		    	else if(j === 0){
		    		var colHead = $('<td><h5>№ экземпляра</h5></td>');
